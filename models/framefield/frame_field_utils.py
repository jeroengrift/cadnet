...
from models.framefield.torch_lydorn.torch.utils.complex import complex_mul, complex_sqrt, complex_abs_squared

# This term is small when the polynomial f(·;bc0,bc2) has a root near eiθτ 
# implicitly implying that one of the field directions {±u, ±v} is aligned with the tangent direction τ
# Both masks are unbalanced so how do we deal with that? Check normalizations
...
def detect_corners(polylines, u, v):
    ...
    def compute_is_corner(points, left_edges, right_edges):
        ...
        coords = np.round(points).astype(np.int64)
